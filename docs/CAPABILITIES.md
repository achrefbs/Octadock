...
| Capture Shelf | Shelf window/cards | `ShelfService`, `ShelfViewModel`, `ShelfItemViewModel`, `ShelfItemView` | Built | Image-first with hover actions, click-to-open image surface, drag/drop, and direct “Build agent task” handoff. Design still needs rendered QA. |
...
| Context | Dock/tray/CLI, shelf/pin add-to-context, Context window, Use with AI source | `ContextService`, `ContextRepository`, `ContextViewModel`, `ContextWindow`, `ContextExport` | Partial | Persistent packages, snapshot/reference ownership, per-item include toggles, safe folder/zip export, verified references, and direct handoff of only included items are built. Missing per-derivative toggles, notes/reorder polish, and MCP. |
...
| Read aloud | Tray, Dock, `Ctrl+Shift+0`, `octadock read`, Use with AI result | `ReadAloudService`, `WindowsTtsProvider`, `ElevenLabsTtsProvider`, `SentenceChunker`, `ReadingPill` | Built | Verbatim by default with Windows voices. `--explain` opens Use with AI with an investigation goal; results retain Read aloud. Screen-discovery overlay remains planned. |
| Use with AI | Dock, tray, Shelf, annotated pin, Context, selected History/Clipboard row, `octadock ai` / `agent` / `handoff` and legacy aliases | `AgentWorkflowCatalog`, `AgentWorkspaceWindow`, `AgentWorkspaceViewModel`, `AgentPacketBuilder`, `AgentPacketExportService`, `AgentCliRunner`, `SkiaVisualComparisonService` | Built | Carries current evidence into Build, Investigate, Verify, Extract, or Handoff and asks only for missing intent. Advanced review builds exact deterministic TASK/manifest/SHA256SUMS packets from captures, Context, clipboard, files, OCR, annotations, acceptance criteria, and before/after verification. Bounded attachments, hashes, safe paths, default text-secret redaction, unchanged-pixel warning, named confirmation, tool-isolated Codex/Claude profiles, no fallback, and ephemeral results are wired. Write-enabled execution, pixel redaction, automatic same-region recapture, MCP, and local-model destinations are not shipped. |
...
| Clipboard history | Hotkey, tray, Dock, CLI/protocol, Settings | `ClipboardMonitor`, `ClipboardHistoryService`, `ClipboardHistoryWindow`, repository | Built | Local text/image clips with provenance, dedupe, favorites, search/filter, copy/delete/clear, trimming, and selected-row Use with AI. Shelf cards for clips remain planned. |
