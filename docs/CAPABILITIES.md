...
| Read aloud | Tray, dock, `Ctrl+Shift+0`, `octadock read` | `ReadAloudService`, `WindowsTtsProvider`, `ElevenLabsTtsProvider`, `SentenceChunker`, `ReadingPill` | Built | Verbatim by default with local Windows voices (offline, keyless, chunked prefetch so audio starts fast); playback pill with pause/stop; `--explain` runs the local AI explainer first; ElevenLabs opt-in; voice/rate/provider in Settings → Read aloud. |
| Settings | Settings window | `SettingsWindow`, `SettingsViewModel`, `SettingsSections` | Partial | Speech, Read aloud, shelf size, local crash-report settings, and the Clipboard history tab are wired; Ask AI/MCP/upload settings still missing. |
