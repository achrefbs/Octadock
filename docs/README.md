...
| [Windows installer](WEB-AND-INSTALLER.md) | Public downloads and per-user Windows Setup |
...
Use `main` for app contributions. The marketing website is maintained privately and hosted on Railway. Historical website commits and the `gh-pages` branch are preserved; they are not the source for future website deployments. Design/recovery branches remain archives, not alternative release instructions.
